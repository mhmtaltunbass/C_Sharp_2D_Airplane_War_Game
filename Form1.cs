...
namespace Ucak_Savasi
{
    public partial class Form1 : Form
    {
        ...
        PictureBox Goktasi= new PictureBox();
        private PictureBox p_goktasi()
        {
            Goktasi= new PictureBox();
            Goktasi.Name = "goktasi";
            Goktasi.Image = global::Ucak_Savasi.Properties.Resources.göktaşı;
            Goktasi.SizeMode=PictureBoxSizeMode.StretchImage;
            Goktasi.Size = new Size(60, 60);
            Goktasi.BackColor = Color.Transparent;
            savas_alani.Controls.Add(Goktasi);
            Random rnd_X = new Random();
            Goktasi.Left = rnd_X.Next(20, 480);
            Random rnd_Y = new Random();
            Goktasi.Top = rnd_Y.Next(100, 1500) * -1;
            return Goktasi;
        }
            ...
            else if (e.KeyCode == Keys.Space) // boşluk tuşu ateş eder.
            {
                ...
                foreach (Control item in savas_alani.Controls)
                {
                    if (item.Name.Contains("mermi"))
                    {
                        ...
                        Mermi.Left = player.Left + 43; // mersinin görsel olarak çıkacağı yer. Uçağın ortasından; 
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (Can > 0) // canlarımız tükenmediyse...
            {
                foreach (Control item in savas_alani.Controls) // savaş alanındaki nesneleri kontrol ediyoruz
                {
                    ...
                    if (item.Name.Contains("goktasi")) // eğer göktaşı adında bir nesne var ise
                    {
                        item.Top += DusmanUcakHareketHiz; // göktaşının hareketi sağlanıyor. 
        ...
        private void basla()
        {
            foreach (Control item in savas_alani.Controls) // savaş alanında kalan uçak ver mermiler kaldırılsın.
            {
                if (item.Name.Contains("dusman")) savas_alani.Controls.Remove(item);
                if (item.Name.Contains("mermi")) savas_alani.Controls.Remove(item);
                if (item.Name.Contains("goktasi")) savas_alani.Controls.Remove(item);
            }
            Can = 5;
            p_dusman(); // bir düşman ekledik
            p_mermi(); // bir mermi ekledik
            p_goktasi(); // bir göktaşı ekledik
            ...
            if (kolayToolStripMenuItem.Checked == true) DusmanUcakHareketHiz = 4;
            if (ortaToolStripMenuItem.Checked == true) DusmanUcakHareketHiz = 6;
            if (zorlukToolStripMenuItem.Checked == true) DusmanUcakHareketHiz = 8;
        ...
        private void oyunSonu()
        {
            ...
            MessageBox.Show("Oyun Bitti! - " + Puan + " Puan Kazandınız....", "DPU Bilgisayar Mühendisliği", MessageBoxButtons.OK, MessageBoxIcon.Information);
            Puan = 0;
            lblsonuc.Text = "0";
            panel1.Enabled = true;
